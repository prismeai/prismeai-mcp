...
The app **lives in the workspace folder itself** — `automations/`, `imports/`,
`index.yml` are the workspace's own files, and `src/`, `scripts/`, `.env`,
`.prismeai/`, `package.json` etc. are added on top.
...
4. Detect the mode:
   - **Bootstrap** : neither `<workspace>/scripts/deploy.mjs` nor `<workspace>/src/App.tsx` exists → run Phase 2A.
   - **Edit** : both files exist → run Phase 2B.
   - **Hybrid** : only one exists → likely corrupted state. Ask the user before continuing (offer re-bootstrap).
...
1. Confirm with the user: "Le workspace `<name>` n'a pas encore d'app React. Je bootstrap depuis github.com/prismeai/starter-spa sur `<env>` ?"
2. Download the starter tarball:
   ...
   ```
3. Copy into the workspace folder, **excluding** `automations/` (already owned by the
   workspace), `.git/`, `.github/`, `.gitlab-ci.yml`, `dist/`, `node_modules/`,
   `package-lock.json`, `.env`, `TODO.md`, `package.json` (templated):
   ```bash
   rsync -av \
     --exclude='automations' \
     --exclude='.git*' \
     ...
     --exclude='src/App.tsx' \
     /tmp/starter-spa-main/ <workspace>/
   ```
4. Render the 3 templates from this skill's `templates/` folder. **All variables
   ...
   |---|---|---|
   | `templates/App.tsx.tpl` | `<workspace>/src/App.tsx` | `{{workspace_name}}` (from `index.yml > name` or folder name) |
   | `templates/package.json.tpl` | `<workspace>/package.json` | `{{name}}` (kebab-case of workspace name), `{{workspace_name}}` |
   | `templates/env.tpl` | `<workspace>/.env` | `{{api_url}}`, `{{access_token}}`, `{{workspace_id}}`, `{{studio_url}}` (all from MCP env) |

   Use `Read` + string replacement, then `Write` to the target. Don't echo
   `access_token` to the conversation.

5. `chmod 600 <workspace>/.env` (defense-in-depth against accidental read).
6. Verify `.gitignore` excludes `.env` (already done by the tarball — `Read` and `Grep` to confirm).
7. Install deps :
   ```bash
...
9. Optionally proceed to a first deploy (Phase 7). For a fresh bootstrap, ask the
   user first — "Premier deploy maintenant ?".

---

## Phase 2B — Defensive state pull (Edit mode)

1. Check `<workspace>/.env` exists. If the token is empty or expired, regenerate
   from the MCP env (same logic as bootstrap step 4, only `.env`).
2. Read `<workspace>/.prismeai/last-pull.json` if present.
3. If missing or older than 24h, do a defensive sync before editing :
   - **Automations + imports + index.yml** : `mcp__prisme-ai-builder__pull_workspace(workspaceId, path: <workspace>)`.
   - **Source files + bundle config** : fetch direct (the MCP doesn't cover these):
     ```bash
     curl -fsS -H "Authorization: Bearer $TOKEN" \
       "$API_URL/workspaces/$WORKSPACE_ID/files?metadata.type=source"
     curl -fsS -H "Authorization: Bearer $TOKEN" \
       "$API_URL/workspaces/$WORKSPACE_ID" | jq '.config.value.bundles'
     ```
     The exact path/payload contract lives in `<workspace>/scripts/pull.mjs` — Read
     it when you need the details.
...
1. Read `<workspace>/AGENTS.md` (authoritative — defers to `scripts/deploy.mjs` for the contract).
2. Read `<workspace>/README.md` (host contract section).
3. Read `<workspace>/src/types.ts` (the `AppProps` contract).
4. Read `<workspace>/src/App.tsx` (entry point).
5. If the request touches automations :
   - `Glob: <workspace>/automations/**/*.yml` → list, read concerned files.
   - Call `mcp__prisme-ai-builder__get_prisme_documentation(section: "automations")` for DSUL syntax.
6. Read `<workspace>/scripts/deploy.mjs` only when you need exact endpoint/payload shapes for direct fetch calls.
...
- **React** : `Edit` / `Write` files under `<workspace>/src/`.
- **Automations** : `Edit` / `Write` YAML files under `<workspace>/automations/`.
...
- **Dependencies** : if you need a new npm package, run `cd <workspace> && npm install <pkg>`. Only add to `externals.mjs` if it's a Prisme.ai runtime module (rare, ask user).
...
| Step | When | How |
|---|---|---|
| Type check | If `src/` was touched | `cd <workspace> && npm run typecheck` |
...
| Build | If `src/` was touched | `cd <workspace> && npm run build` (skip if only YAML changed) |
...
| File under `src/**` changed | **curl** | Mirror `scripts/deploy.mjs > collectSourceFiles`: 6 root files (`package.json`, `tsconfig.json`, `vite.config.ts`, `tailwind.config.js`, `postcss.config.js`, `index.html`) + everything under `src/**` (except `.yml`/`.yaml` and files matching `* copy*`). For each: SHA-256, GET `/workspaces/:id/files?metadata.path=<path>&metadata.type=source` to find existing, DELETE old if hash differs, then POST multipart with `metadata.path` + `metadata.type=source` + `metadata.hash` + `public=false`. **This is NOT optional** — without it, the in-builder Builder shows only the read-only bundle, with no editable source side. |
...
curl -fsS -X POST "$API_URL/workspaces/$WORKSPACE_ID/files" \
  ...
  -F "file=@<workspace>/dist/bundle.js;filename=bundle.js;type=application/octet-stream" \
  -F "public=true"
# → grab .url and .id from the response[0]

# 3. Upload a source file (private)
HASH=$(shasum -a 256 <workspace>/src/App.tsx | cut -d' ' -f1)
curl -fsS -X POST "$API_URL/workspaces/$WORKSPACE_ID/files" \
  -H "Authorization: Bearer $TOKEN" \
  -F "file=@<workspace>/src/App.tsx;filename=App.tsx;type=text/plain" \
...
#    Reconstruct config.value from the local index.yml (yq or python yaml),
...
1. Build the access URL : the `studioUrl` (Phase 2) + `/apps/<workspace-slug>`. Print it as a clickable link so the user can open it.
2. Report :
   - Workspace ID + name
   - Env (sandbox / staging / prod)
   - Version (`PRISMEAI_APP_VERSION`)
   - Automations touched : counts (created / updated / deleted)
   - Source files pushed : count + bundle size
3. If the workspace folder is under git, suggest a commit. **Don't commit
   automatically.**
