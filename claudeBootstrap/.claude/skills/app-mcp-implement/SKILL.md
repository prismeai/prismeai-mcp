...
16. **`validate_automation` is authoritative.** Run it on every automation. `push_workspace` version name ≤ 15 chars. Nested expressions need inner `{% %}`; `replace(...)` all-occurrences via JS `split().join()` in Custom Code; `matches/and/or` only at condition level.
