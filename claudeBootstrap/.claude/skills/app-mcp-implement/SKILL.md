...
- `onInstall.yml`: provisions tenant secrets `<camel>Auth` (object) + `<camel>AuthorizedAgents`; writes binding B (workspace `config.value` via minted workspace JWT + `makeSecretRef`) then binding A (terminal `set: config` merge via `makeConfigRef`); sets `configAppUrl` + (OAuth connectors only) `oauthCallbackUrl`. **All generated URLs use the raw tenant workspace id (`{{global.workspaceId}}`), NEVER `slug:<slug>`** — the id is stable for the life of the workspace, so a slug rename never staleifies a frozen URL (a slug-based URL 404s after rename). Do NOT store `mcpEndpoint` on the instance config — the SPA derives it client-side from the `workspaceId` it receives via `configAppUrl`. Idempotent.
- `index.yml`: `config.schema` (configAppUrl readOnly only — NO mcpEndpoint field; apiVersion if any), `config.value` (apiVersion, the two binding aliases, `bundles[<slug>]` for the SPA), `secrets.schema` (`<camel>Auth` object + `<camel>AuthorizedAgents`), `mcpTools` (filled in Phase 5). `labels` incl. `app-mcp`.
...
16. **Generated endpoints use the raw tenant id, NEVER the slug.** In `onInstall`, build every stored/exposed endpoint — `configAppUrl`'s `workspaceId=` query, `oauthCallbackUrl` (OAuth connectors), and the internal `secrets`/`config` URLs — from `{{global.workspaceId}}` (set `wsRef = {{global.workspaceId}}` and reuse it). The id is stable for the life of the workspace; a `slug:<slug>`-based URL is **frozen at install time and 404s the moment an admin renames the tenant slug** (it also breaks OAuth exact-match — the provider compares `redirect_uri` byte-for-byte across authorize, token exchange, and console registration). Do NOT mint a JWT just to `GET /workspaces/:id` for the slug — that whole dance is removed. **Never store `mcpEndpoint` on the instance config** (no schema field, no `set`, no emit/output): the config SPA derives it client-side from the `workspaceId` it gets via `configAppUrl`, and exposes it (display + "Install capability"). A re-import that changes the workspace id re-runs `onInstall` (apps.installed) anyway, so it regenerates the id-based URLs — the id approach loses nothing and fixes slug-rename.
